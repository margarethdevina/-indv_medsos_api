...
module.exports = {
    getData: async (req, res, next) => {
        try {
            let getData = await dbQuery(`Select p.id, u.username, p.media, p.caption, p.uploadDate, p.editedDate, count(l.postId) as numberOfLikes from posts p left join users u on p.userId = u.id left join likes l on p.id = l.postId group by p.id;`)
    ...
    add: async (req, res, next) => {
        try {
            ...
            let insertPost = await dbQuery(`INSERT INTO posts (userId, media, caption) VALUES (${dbConf.escape(req.body.userId)}, ${dbConf.escape(req.body.media)}, ${dbConf.escape(req.body.caption)});`);
    ...
    delete: async (req, res, next) => {
        try {
            ...
            let deletePost = await dbQuery(`Delete from posts where id = ${dbConf.escape(req.params.id)};`);

            let getLikes = await dbQuery(`SELECT * FROM likes where postId = ${dbConf.escape(req.params.id)};`);

            if (getLikes.length > 0){
                let deleteLikesQuery = `DELETE from likes where id in (`
                let likesIdQuery = ``
            ...
            if (getComments.length>0){
                let deleteCommentsQuery = `DELETE from comments where id in (`
                let commentsIdQuery = ``
